...
import { Layers, Percent, Download, PieChart, ChevronLeft, ChevronRight, Search, X } from "lucide-react";
...
const StockUsageChartYear = () => {
  ...
  useEffect(() => {
    setLoading(true);
    setError("");
    axiosInstance.get("/simple-stock-usage").then((res) => {
      const raw = res.data || [];
      const map = {};
      for (const item of raw) {
        const m = (item.month || 1) - 1;
        const name = item.stock_name || "Bilinmiyor";
  ...
  const filteredSeries = useMemo(() => {
    const names = new Set(selectedStocks);
    return series.filter((s) => names.has(s.name));
  }, [series, selectedStocks]);
  ...
  const topStockTrend = useMemo(() => {
    const found = series.find(s => s.name === topStock);
  ...
  const toggleStock = (name) => {
    setSelectedStocks((prev) => prev.includes(name) ? prev.filter(n => n !== name) : [...prev, name]);
  };
  const selectAll = () => setSelectedStocks(stockNames);
  const clearSelection = () => setSelectedStocks([]);

  const exportCsv = () => {
    const { min, max } = windowRange;
    const cats = monthNames.slice(min, max + 1);
    const header = ["Stok", ...cats];
      ...
      <div className="identity-panel-banner" style={{ 
        ...
        <div className="identity-header-stats" style={{ gap: 12 }}>
          <div className="identity-stat-pill" style={{ background: "rgba(255, 255, 255, 0.2)", border: "none" }}>
            📦 Toplam: <strong style={{ color: "#fff", fontSize: "1.1em" }}>{grandTotal}</strong>
          </div>
          <div className="identity-stat-pill" style={{ background: "rgba(255, 255, 255, 0.2)", border: "none" }}>
          ...
          <div className="identity-stat-pill" style={{ background: "rgba(255, 255, 255, 0.2)", border: "none" }}>
            🏆 En Çok: <strong style={{ color: "#fff", fontSize: "1.1em" }}>{topStock}</strong>
          </div>
        </div>
      </div>
            ...
            <aside className="identity-sidebar" style={{ alignSelf: "stretch", position: "static", height: "100%" }}>
              <div className="identity-section-card compact" style={{ 
                ...
                <div className="identity-checkbox-grid" style={{ 
                  ...
                  {stockNames.filter(n => n.toLowerCase().includes(stockFilter.toLowerCase())).map((n) => (
                ...
                <div style={{ 
                  ...
                  {selectedStocks.map((n) => (
                    <span key={n} className="identity-stat-pill" style={{ 
                      ...
                      📦 {n}
                      <button className="identity-icon-btn" onClick={() => toggleStock(n)} style={{ 
                        marginLeft: 4,
                        color: "white",
                        opacity: 0.8
                      }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 16, marginTop: 20 }}>
                    <div className="identity-section-card compact" style={{ 
                      ...
                      <Chart type="line" width="100%" height={120} options={{ 
                        ...
                        markers: { size: 4 }
                      }} series={[{ name: topStock, data: topStockTrend }]} />
                    </div>
                  </div>
                </div>
              </div>
...
export default StockUsageChartYear;
