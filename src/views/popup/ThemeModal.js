    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm" PaperProps={{ sx: { borderRadius: 3, boxShadow: '0 20px 60px rgba(0,0,0,0.15)' } }}>
      <DialogTitle sx={{ p: 2.5 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Typography variant="h6" fontWeight={700}>Tema</Typography>
        </Box>
      </DialogTitle>
      <DialogContent dividers sx={{ p: 2.5 }}>
        <Stack spacing={2}>
          <FormControlLabel control={<Switch checked={dark} onChange={(e) => setDark(e.target.checked)} />} label="Karanlık Mod" />
      ...
      <DialogActions sx={{ p: 2, justifyContent: 'space-between' }}>
        ...
        <Button onClick={() => onSave?.({ dark, primary })} variant="contained" sx={{ borderRadius: 2, background: 'linear-gradient(135deg,#4facfe,#00f2fe)' }}>Kaydet</Button>
      </DialogActions>
    </Dialog>
  );
};
